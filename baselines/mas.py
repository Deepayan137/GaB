...
class MAS(nn.Module):
    ...
    def _get_importance(self, model, dloader, optimizer, device):
    ...
    def after_training_exp(self, model, dloader, optimizer, device, **kwargs):
        self.params = dict(copy_params_dict(model))

        # Get importance
        if self.importance is None:
            self.importance = self._get_importance(model, dloader, optimizer, device)
            return
        else:
            curr_importance = self._get_importance(model, dloader, optimizer, device)
