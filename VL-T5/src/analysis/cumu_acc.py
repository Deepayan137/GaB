...
class Analysis(object):
    def __init__(self, paths):
        ...
        self.get_matrix(paths_with_pred)

    def _check_pred_dir_exists(self, paths):
        # Create a list of tuples (path, has_predictions)
        paths_with_predictions = [(os.path.join(path, "predictions")) for path in paths]
    ...
    def _check_if_causal_exists(self, paths):
        with_causal = [os.path.join(path, "predictions") for path in paths if os.path.exists(os.path.join(path[0], "predictions", "q_causal_acc.json"))]
        return with_causal
    ...
    def get_matrix(self, paths):
        for path in paths:
            ...
            target_path = os.path.join("acc_metrics", f"{model_name}.csv")
