...
class Blip2VQACL(Blip2ForConditionalGeneration):
            ...
            if current_task_id != 0:
                if current_task_id not in self.Q_task_mem_proto:
                    ...
                    self.Q_task_mem_proto[current_task_id] = (
                        proto_alpha * self.Q_task_mem_proto[current_task_id] + (1 - proto_alpha) * current_Q_prototype_mem.detach()
                    )
    ...
    def calculate_current_prototype(self, fc_hidden_Q, labels):
        ...
        current_prototype_Q = torch.matmul(torch.transpose(labels, 0, 1), fc_hidden_Q) / div_item  # [num_classes1, dim]
    ...
    def forward(
        ...
        **kwargs,
        ...
        if "proto_update" in kwargs and kwargs["proto_update"]:  # only for training
            current_prototype_Q, current_num_Q = self.calculate_current_prototype(input_embeds.to(language_model_inputs.device), ques_labels)
            current_prototype_V, current_num_V = self.calculate_current_prototype(language_model_inputs, cate_labels)
            # import pdb;pdb.set_trace()
            if "memory" in kwargs and kwargs["memory"] == True:
                loss_memory_Q, loss_memory_V = self.memory_loss(input_embeds.to(language_model_inputs.device), language_model_inputs, ques_labels, cate_labels)
            else:
            ...
            retrievaled_Q_proto, max_idx_Q, acc_Q = self.cosine_similarity_multi(self.Q_prototype, torch.mean(input_embeds, dim=1), ques_labels)  # [bs, 768]
            ...
            retrievaled_V_proto, max_idx_V, acc_V = self.cosine_similarity_multi(
                self.V_prototype, torch.mean(language_model_inputs, dim=1), cate_labels
            )  # [bs, 768]
        ...
        inputs_embeds = torch.cat(
            [
                ...
                retrievaled_Q_proto.detach(),
                retrievaled_V_proto.detach(),
            ],
            dim=1,
        )
