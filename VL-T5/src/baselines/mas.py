    ...
    def before_backward(self, model, **kwargs):
        ...
        for name, param in model.named_parameters():
            if name in self.importance.keys():
                loss_reg += torch.sum(self.importance[name].expand(param.shape) * (param - self.params[name].expand(param.shape)).pow(2))
    ...
    def after_training_exp(self, model, optimizer, dloader, **kwargs):
        ...
        for name in curr_importance.keys():
            ...
            if name not in self.importance:
                ...
                self.importance[name].data = self.alpha * self.importance[name].expand(new_shape) + (1 - self.alpha) * curr_importance[name].data
