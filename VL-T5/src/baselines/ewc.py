...
class EWC(nn.Module):
    ...
    def __init__(self, ewc_lambda, decay_factor=None, uniform_importance=False):
        """
    ...
    def before_backward(self, model, **kwargs):
        ...
        for k, cur_param in model.named_parameters():
            ...
            penalty += (imp.expand(new_shape) * (cur_param - saved_param.expand(new_shape)).pow(2)).sum()
    ...
    def after_training_exp(self, model, optimizer, dloader, **kwargs):
        ...
        if self.uniform_importance and len(self.importances) == 0:
            ...
            importances = self.compute_importances(model, optimizer, dloader, **kwargs)
            self.update_importances(importances)
        self.saved_params = copy_params_dict(model)

    def compute_importances(self, model, optimizer, dataloader, **kwargs) -> Dict[str, ParamData]:
        """
        ...
        for i, batch in enumerate(tqdm(dataloader, desc="Computing weights relevance")):
            ...
            for (k1, p), (k2, imp) in zip(requires_grad(model.named_parameters()).items(), importances.items()):
                assert k1 == k2
    ...
    def get_state_dict(self):
        ...
        s_dict = {"params": params_dict, "importance": importance_dict}
        return s_dict
...
if __name__ == "__main__":
    ...
    num_classes = 10
    num_epochs = 5
    model = nn.Sequential(nn.Linear(1, 32), nn.ReLU(), nn.Linear(32, 32), nn.ReLU(), nn.Linear(32, num_classes))
