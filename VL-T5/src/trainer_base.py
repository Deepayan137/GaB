...
class TrainerBase(object):
    ...
    def create_model(self, model_class, config=None, **kwargs):
        ...
        model = model_class.from_pretrained(model_name, cache_dir=".", config=config, device_map="auto", trust_remote_code=True, **kwargs)
        return model

    def create_tokenizer(self, **kwargs):
        processor = AutoProcessor.from_pretrained(self.args.backbone, max_length=self.args.max_text_length, do_lower_case=self.args.do_lower_case, **kwargs)
    ...
    def create_optimizer_and_scheduler(self, total_train_num):
        ...
        if "blip_adamw" in self.args.optim:
            ...
            optim = torch.optim.AdamW(params=params, lr=self.args.lr, weight_decay=self.args.warmup_ratio)
        else:
