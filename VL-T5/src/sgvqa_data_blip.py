...
class SGVQA_memory(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target_ids = self.processor.tokenizer.encode(out_dict["answer"], max_length=10, truncation=True)
        out_dict["target_ids"] = torch.LongTensor(target_ids)
        out_dict["target_length"] = len(target_ids)
        return out_dict
...
def sample_by_labels(args, predictions, split, sub_task, task):
    ...
    desired_task_counts = {int(k): math.ceil(v * split / 100.0) for k, v in desired_task_counts.items()}
...
def get_loader_memory(
    ...
    def cat_loader():
        dataset = SGVQA_memory(Examplar_set, split=split, verbose=verbose, args=args)
        if split == "train":
...
def get_loader(args, split="train", scenario="scene", batch_size=32, workers=4, task="object"):
    verbose = True
...
def get_loader_test(args, split="train", scenario="scene", batch_size=32, workers=4, task="what"):
    verbose = True
    dataset = SGVQA(split=split, verbose=verbose, args=args, scenario=scenario, task=task)  # all categories
